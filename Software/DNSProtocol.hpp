...
// This file contains several memory leaks that transfered over from SimpleDNS valgrind --leak-check=full 
...
//using creates a better version of typedef. These are now types of the given function signitures below
using UpdateRecordCallback_t = std::function<bool(const std::string&, const std::vector<uint8_t>& )>;
using GetRecordCallback_t = std::function<const std::vector<uint8_t>(const std::string& )>;
