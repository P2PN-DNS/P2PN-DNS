...
// For every question in the message add a appropiate resource record
// in either section 'answers', 'authorities' or 'additionals'.
struct Message resolver_process(struct Message* msg, std::fuction<int>(uint8_t addr[4], const char domain_name[]), std::fuction<int>(uint8_t addr[16], const char domain_name[])   );
{
  struct ResourceRecord* beg;
  struct ResourceRecord* rr;
  struct Question* q;
